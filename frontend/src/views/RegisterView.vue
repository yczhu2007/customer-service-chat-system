...
async function submit() {
  ...
  try {
    const result = await register({ username: form.value.username.trim(), password: form.value.password })
...
<template>
  <section class="account-screen"><el-form ref="formRef" class="account-card" :model="form" :rules="rules" label-position="top" @submit.prevent="submit">
    ...
    <el-alert v-if="success" class="success" :title="success" type="success" :closable="false" />
    <el-button native-type="submit" type="primary" :loading="submitting">{{ submitting ? '提交中…' : '注册' }}</el-button>
