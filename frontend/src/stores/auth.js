...
export const useAuthStore = defineStore('auth', {
  state: () => ({ token: null, userId: null, username: null, nickname: null, roles: [], activeRole: null, role: null }),
  getters: {
    displayName: (state) => state.nickname || '未设置昵称',
  ...
  actions: {
    restore() {
      try {
        ...
        Object.assign(this, { token: saved.token, userId: saved.userId, username: saved.username || saved.userId, nickname: saved.nickname || '未设置昵称', roles, activeRole, role: activeRole })
      } catch { this.logout() }
    },
    login(payload) {
      ...
      this.nickname = payload.nickname || '未设置昵称'
      this.roles = roles
    ...
    persist() {
      localStorage.setItem(KEY, JSON.stringify({ token: this.token, userId: this.userId, username: this.username, nickname: this.nickname, roles: this.roles, activeRole: this.activeRole }))
    },
    logout() {
      this.$reset()
