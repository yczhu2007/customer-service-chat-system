...
async function openResult(result) { if (!chat.sessions.some((item) => item.sessionId === result.sessionId)) chat.sessions.unshift({ sessionId: result.sessionId, title: result.sessionTitle || '新咨询', status: result.sessionStatus, userId: result.userId, agentId: result.agentId }); await chat.selectSession(result.sessionId); emit('selected') }
