...
async function openResult(result) { if (!chat.sessions.some((item) => item.sessionId === result.sessionId)) chat.sessions.unshift({ sessionId: result.sessionId, title: result.sessionTitle || result.sessionId, status: result.sessionStatus, userId: result.userId, agentId: result.agentId }); await chat.selectSession(result.sessionId); emit('selected') }
function clear() { keyword.value = ''; results.value = []; total.value = 0; error.value = ''; hasSearched.value = false }
</script>
<template><section class="message-search-panel"><div class="search-row"><el-input v-model="keyword" clearable placeholder="&#25628;&#32034;&#25105;&#30340;&#21382;&#21490;&#28040;&#24687;" @keyup.enter="search"/><el-button type="primary" :loading="loading" :disabled="!keyword.trim()" @click="search">&#25628;&#32034;</el-button><el-button :disabled="!keyword && !results.length" @click="clear">&#28165;&#31354;</el-button></div><p v-if="error" class="search-error">{{ error }}</p><p v-else-if="hasSearched && !loading" class="summary">&#20849;&#21305;&#37197; {{ total }} &#26465;&#28040;&#24687;</p><div v-if="results.length" class="search-results"><button v-for="result in results" :key="result.messageId" class="search-result" @click="openResult(result)"><strong>{{ result.sessionTitle || result.sessionId }}</strong><span>{{ result.content }}</span><small>{{ result.senderRole === 'AGENT' ? '&#23458;&#26381;' : '&#29992;&#25143;' }} · {{ result.createTime }}</small></button></div></section></template>
